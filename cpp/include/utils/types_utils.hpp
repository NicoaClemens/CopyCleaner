...
inline bool operator==(const RuntimeValue& a, const RuntimeValue& b) {
    ...
    switch (a.value.index()) {
        ...
        case 4:  // List
            return std::get<RuntimeValue::List>(a.value).values ==
                   std::get<RuntimeValue::List>(b.value).values;
        case 5: {  // Match
        ...
        [&](const auto& val) -> bool {
            ...
            return std::visit(
                [&](const auto& ty) -> bool {
                    ...
                    if constexpr (std::is_same_v<V, RuntimeValue::Int> &&
                                  std::is_same_v<T, AstType::Int>)
                        return true;

                    else if constexpr (std::is_same_v<V, RuntimeValue::Float> &&
                                       std::is_same_v<T, AstType::Float>)
                        return true;

                    else if constexpr (std::is_same_v<V, RuntimeValue::Bool> &&
                                       std::is_same_v<T, AstType::Bool>)
                        return true;

                    else if constexpr (std::is_same_v<V, RuntimeValue::String> &&
                                       std::is_same_v<T, AstType::String>)
                        return true;

                    else if constexpr (std::is_same_v<V, RuntimeValue::Regex> &&
                                       std::is_same_v<T, AstType::Regex>)
                        return true;

                    else if constexpr (std::is_same_v<V, RuntimeValue::Match> &&
                                       std::is_same_v<T, AstType::Match>)
                        return true;

                    else if constexpr (std::is_same_v<V, RuntimeValue::List> &&
                                       std::is_same_v<T, AstType::List>) {
                        // If the AST list type specifies an element type, ensure all elements match
                        // it
                    ...
                    else if constexpr (std::is_same_v<V, RuntimeValue::Null> &&
                                       std::is_same_v<T, AstType::Null>)
                        return true;

                    // implicit numeric conversions
                    else if constexpr (std::is_same_v<V, RuntimeValue::Int> &&
                                       std::is_same_v<T, AstType::Float>)
                        return true;
