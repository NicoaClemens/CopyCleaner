...
Result<RuntimeValue> Interpreter::run(std::vector<Statement>& stmts) {
    ...
    return std::visit(overloaded{
        ...
        [](ExecFlow::Break) -> Result<RuntimeValue> {
            return err<RuntimeValue>(std::make_shared<Error>("invalid 'break' statement", ErrorKind::Syntax));
        },
        [](ExecFlow::Continue) -> Result<RuntimeValue> {
            return err<RuntimeValue>(std::make_shared<Error>("invalid 'continue' statement", ErrorKind::Syntax));
        }
    }, exec.value);
}
...
Result<ExecFlow> Interpreter::eval_statements(std::vector<Statement>& stmts, Environment& env) {
    for (auto &s : stmts) {
        ...
        if (std::holds_alternative<Statement::Break>(s.value)) {
            ExecFlow out; out.value = ExecFlow::Break{}; return ok(out);
        }

        // Continue
        if (std::holds_alternative<Statement::Continue>(s.value)) {
            ExecFlow out; out.value = ExecFlow::Continue{}; return ok(out);
        }
    }
...
Result<RuntimeValue> Interpreter::eval_expr(Expr& expr, envPtr env) {
    ...
    auto make_err = [&](std::shared_ptr<Error> e)-> Result<RuntimeValue> { return err<RuntimeValue>(e); };
    ...
    if (std::holds_alternative<E::FunctionCall>(expr.value)) {
        ...
        if (it != this->functions.end()) {
            ...
            if (m.args.size() != eval_args.size()) return err<RuntimeValue>(std::make_shared<Error>("argument count mismatch", ErrorKind::Arity));
            ...
            for (auto &kv : m.args) {
                ...
                if (!matches_type(eval_args[idx], pty)) return err<RuntimeValue>(std::make_shared<Error>("argument type mismatch", ErrorKind::Type));
